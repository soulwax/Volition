...
Thanks for your interest in contributing to Volition! :tada: We love getting [pull requests](https://www.quora.com/GitHub-What-is-a-pull-request) for bugfixes and contributions of our community to keep Volition growing.
...
As soulwax creates his videos about Volition in English, the devs ask to keep the code base, issues and pull requests in English only.
...
You've created a new fix or feature for Volition. Awesome!

1. If you haven't already, create a fork of the Volition repository.
...
4. Give me a moment. Volition is maintained by only a few people, all of whom are doing this on their limited free time, so it may take us a bit to review your request. Bug fixes should be merged in directly.
...
- Naming convention:
  ...
  - For macros we use snake case: **`MACRO_NAME`**.
    - If it is specifically related to Volition, we add the 'VL_' prefix: **`VL_MACRO_NAME`**.
    - If there is a macro for the application and for the engine, we add an additional 'CORE_' prefix to the engine macro:  **`VL_CORE_MACRO_NAME`**.
...
[issue-tracker]: https://github.com/soulwax/Volition/issues
[submit-pr]: https://github.com/soulwax/Volition/pulls
